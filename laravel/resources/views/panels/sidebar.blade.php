<aside
    ...
    <ul class="sidenav sidenav-collapsible leftside-navigation collapsible sidenav-fixed menu-shadow" id="slide-out"
        ...
        $activeClass=($routeName==$menu->slug)?'active':'';
        $activeClassWithMenuColor=($routeName==$menu->slug)?'active '.$configData['activeMenuColor']:'';
        @endphp
        <li class="bold {{$activeClass}}">
            <a class="{{$custom_classes}} {{ $activeClassWithMenuColor}}" @if(!empty($configData['activeMenuColor']))
                {{'style=background:none;box-shadow:none;'}} @endif
        ...
        @endif
        <li class="bold ">
            <a class="waves-effect waves-cyan " style="background:none;box-shadow:none;"
                href="{{env('CHAT_URL')}}/{{Auth::user()->id}}">
                <i class="material-icons">chat_bubble_outline</i>
                <span class="menu-title">{{__('locale.chat')}}</span>
            </a>
        </li>
    </ul>
